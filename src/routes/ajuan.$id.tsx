...
function DetailAjuanPage() {
  ...
  const handleDownloadPdf = async (type: 'nota' | 'administrasi') => {
    const targetId = type === 'administrasi' ? "form-administrasi-document" : "nota-document";
    const element = document.getElementById(targetId);
    if (!element) return;
    toast.info(`Menyiapkan ${type === 'administrasi' ? 'Formulir' : 'Nota'} Anggaran PDF...`, { duration: 2000 });
    try {
      ...
      const pdf = new jsPDF("p", "mm", type === 'administrasi' ? "a4" : "a5");
      ...
      const filename = type === 'administrasi' ? `Form_Pencairan_${safeKode}.pdf` : `Nota_${safeKode}.pdf`;
      pdf.save(filename);
      element.style.cssText = prev;
      toast.success(`${type === 'administrasi' ? 'Formulir' : 'Nota'} Anggaran berhasil diunduh!`);
    } catch (err: any) {
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <button onClick={() => handleDownloadPdf('nota')} className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-soft hover:bg-primary/90 print:hidden"><Download className="h-4 w-4" /> Unduh Nota PDF</button>
            <button onClick={() => handleDownloadPdf('administrasi')} className="inline-flex h-10 items-center gap-2 rounded-lg border border-primary/20 bg-primary/5 px-4 text-sm font-semibold text-primary hover:bg-primary/10 print:hidden"><FileText className="h-4 w-4" /> Form Administrasi</button>
